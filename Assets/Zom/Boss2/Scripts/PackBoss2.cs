...
public class PackBoss2 : MonoBehaviour
{
    private Animator animator;
    [SerializeField] public GameObject[] h_BulletEnemy;
    [SerializeField] float h_TimeAttack = 2;
    ...
    public void Attack2()
    {
        ...
        int randomNumber = Random.Range(0, h_BulletEnemy.Length);
        GameObject game = Instantiate(h_BulletEnemy[randomNumber], vector3, Quaternion.identity);
        AudioManager.instance.Play("EnemyAttack");
        animator.SetInteger("State", 0);
    }
    ...
    public void FindPlayer2()
    {
        ...
        if (player01Animation != null)
        {
            ...
            if (direction <= 20 && h_TimeAttack <= 0)
            {
