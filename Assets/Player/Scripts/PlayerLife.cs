...
public class PlayerLife : FindObject
{
    ...
    /*public void Hit()
    {
        StartCoroutine(DecreaseHpOverTime(5f, 3f));
    }

    IEnumerator DecreaseHpOverTime(float percentage, float duration)
    {
        float decreasePerSecond = (GetTotalHP() * (percentage / 100f)) / duration;
        float elapsedTime = 0f;
        int decreaseAmountPerSecond = Mathf.RoundToInt(decreasePerSecond);
        while (elapsedTime < duration)
        {
            yield return new WaitForSeconds(1f);
            h_CurrentHp -= decreaseAmountPerSecond;
            elapsedTime += 1f;
            GameObject point = Instantiate(h_HpPopUp[0], new Vector3(transform.position.x, transform.position.y + 0.5f, transform.position.z), quaternion.identity);
            point.transform.GetChild(0).GetComponent<TextMeshProUGUI>().text = "-" + (decreaseAmountPerSecond);

            Animator animator = GetComponent<Animator>();
            animator.SetTrigger("IsHurt");
        }

        // Đảm bảo HP không bị vượt quá giá trị tối đa
        if (h_CurrentHp < 0f)
        {
            h_CurrentHp = 0f;
        }
